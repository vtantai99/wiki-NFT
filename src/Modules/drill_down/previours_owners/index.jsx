...
import { useDrillDown, useWikiCollect } from 'Hooks'
...
const PreviousOwner = () => {
  ...
  const { changePreviousOwnersAction, previousOwners } = useDrillDown()
  const { data, isSubmitting } = previousOwners

  const onSubmit = (formData) => {
    changePreviousOwnersAction({ data: formData, name: 'notable holders', callback: () => setIsEdit(false) })
  }
  ...
  return (
    ...
    >
      <TextNormal fontWeight="fw_700">Previous Owners:</TextNormal>
      <Wrapper>
        <FormProvider {...form}>
