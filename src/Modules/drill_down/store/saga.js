...
export function* changePreviousOwnersSaga({ payload })
{   const { data, name, callback } = payload
    try {
        yield call(delay)
        yield put({
            type: SUCCESS(CHANGE_PREVIOUS_OWNERS),
            data
        })
        notification.success({
            message: CHANGE_SUCCESS(name)
        ...
        yield put({
            type: FAILURE(CHANGE_PREVIOUS_OWNERS),
            error
        })
    }
}
...
export default function* wikiCollectSaga()
{
    ...
    yield takeLatest(REQUEST(CHANGE_PREVIOUS_OWNERS), changePreviousOwnersSaga)
}